...
def createdAt(list, *args):
    ...
    return [
        time_list.index(min(time_list)),
        min(time_list).strftime("%Y-%m-%dT%H:%M:%SZ"),
    ]


# return the index and value of the maximum lead score unless the score is highly negative,
# then return this negative value (for bad quality leads)
...
# define the prioritized values for certain lead fields in order of decreasing priority
# from left to right
priority_dict = {
    "website": [".com", ".net", ".org"],
...
# use the priority_dict function to find the highest priority value and corresponding index
# among the input lead values for a certain field. Else if none of the input lead
# values has a prioritized value return the first non-null value and index
...
# return the first TRUE value and its index. True is prioritized because this function is used
# for the unsubscribed account blocked fields where it is important to favor TRUE over FALSE
...
# reorder based on client's request
rules = {
    "earliestCreateDate": createdAt,
    "createdAt": createdAt,
    "sfdcLeadId": notNull,
    "firstName": notNull,
    "lastName": notNull,
...
# create a new list with None converted to string format and then pass this list
# to the rules dictionary so the correct prioritzation function is used on the lead
